...
theorem regeneration_hierarchy_mahlo_unconditional :
    ∀ (n : ℕ) (r : OrdinalRegenerationLevel)
      ...
      (h_cof : Ordinal.omega < ((ordinalNextLevel^[n+1]) r).level.card.ord),
...
theorem separation_theorem {n : ℕ} (hn : n < 33) (hn_pos : 0 < n)
    ...
    M.trace ≠ 33 := by
  -- Same proof as PrincipiaVol1.separation_theorem: bound the off-zeroth
  -- diagonal sum by (n-1)·exp(-12) < 32·exp(-12) < 1, hence |trace - 1| < 1.
  have trace_split : M.trace =
      M ⟨0, hn_pos⟩ ⟨0, hn_pos⟩ + ∑ i ∈ Finset.univ.erase ⟨0, hn_pos⟩, M i i := by
    rw [Matrix.trace, ← Finset.sum_erase_add _ _ (Finset.mem_univ ⟨0, hn_pos⟩)]
    ring
  have hexp_pos : (0 : ℝ) < Real.exp (-12) := Real.exp_pos _
  have hexp_bound : Real.exp (-12) < 1 / 32 := by
    have h1 : Real.exp (-12 : ℝ) ≤ Real.exp (-1 : ℝ) :=
      Real.exp_le_exp.mpr (by norm_num)
    have h2 : Real.exp (-1 : ℝ) < 1 / 2 := by
      have he1 : Real.exp 1 > 2 := by
        have := Real.exp_one_gt_d9; linarith
      have hpos : (0 : ℝ) < Real.exp 1 := Real.exp_pos _
      have heq : Real.exp (-1) = 1 / Real.exp 1 := by rw [Real.exp_neg]
      linarith [heq, hpos]
    linarith
  have remainder_bound : |∑ i ∈ Finset.univ.erase ⟨0, hn_pos⟩, M i i| < 1 := by
    calc |∑ i ∈ Finset.univ.erase ⟨0, hn_pos⟩, M i i|
      ...
      _ ≤ ∑ i ∈ Finset.univ.erase ⟨0, hn_pos⟩, Real.exp (-12) := by
      ...
      _ < 1 := by
          have hn_le : (n - 1 : ℝ) ≤ 32 := by
            have : (n : ℝ) ≤ 32 := by exact_mod_cast Nat.lt_succ_iff.mp hn
            linarith
          have key : (n - 1 : ℝ) * Real.exp (-12) ≤ 32 * Real.exp (-12) := by
            nlinarith
          have : (32 : ℝ) * Real.exp (-12) < 1 := by linarith
          linarith
  have trace_close : |M.trace - 1| < 1 := by
    rw [trace_split, h00]; convert remainder_bound using 1; ring_nf
  intro hbad
