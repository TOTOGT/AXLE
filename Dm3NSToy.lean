-- NOTE (fixed): the original two imports below --
...
lemma iterate_to_attractor (X : NSState) :
  ...
  have h : (nsStep^[X.energy] X).energy = nsAttractor.energy := by
    simp only [nsAttractor]
    rw [iterate_nsStep_energy]
    omega
  calc nsStep^[X.energy] X = ⟨(nsStep^[X.energy] X).energy⟩ := rfl
    _ = ⟨nsAttractor.energy⟩ := by rw [h]
    _ = nsAttractor := rfl
...
example : (nsStep^[3] ⟨7⟩).energy = 4 := by
  have h := iterate_nsStep_energy (⟨7⟩ : NSState) 3
  simpa using h
