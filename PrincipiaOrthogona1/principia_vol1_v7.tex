...
\title{%
  ...
  \large Second Edition --- Version~7, August 2026%
}
...
Forty-nine arithmetic and structural facts underlying the framework,
including Theorems~A--D (existence, closure, contact normal form,
structural stability), are machine-checked in Lean~4 against a pinned
Mathlib4 revision, with no axiom beyond \texttt{propext},
\texttt{Classical.choice} and \texttt{Quot.sound}.
Section~\ref{sec:status} states exactly which claims that covers and
which it does not.
\end{abstract}
...
\small
\textbf{Correction notice --- Version 7, 24 August 2026.}
Versions 3 through 6 of this deposit shipped a Lean~4 file,
\texttt{PrincipiaVol1.lean}, described as \emph{``30+ facts proved, 1
sorry (clearly scoped), 0 axioms beyond Mathlib4.''}
That description had never been checked. The file had not once been
elaborated by Lean; its first build, on 24 August 2026 and under the
toolchain this deposit itself names, reported \textbf{81 errors}.
The faults were mechanical --- structure fields separated by
\texttt{;}, a \texttt{MetricSpace} passed where a \texttt{Dist} was
expected, four Ordinal lemmas invoked under names that do not exist ---
but the consequence was not: nothing those versions claimed about the
file had been verified by anything.
\smallskip

The one advertised \texttt{sorry} sat in a \emph{separation theorem},
attributed to a missing Mathlib eigenvalue API. That attribution was
also wrong. The statement as deposited is \textbf{false} --- its
hypothesis bounds only the transverse diagonal of the matrix, so at
dimension~1 it is vacuous and the $1\times 1$ matrix $(33)$ refutes it
--- and the intended argument concerned $\operatorname{Tr}(M^6)$, not
$\operatorname{Tr}(M)$; the exponent had been dropped in transcription.
No upstream API would have closed that gap.
\smallskip

V7 makes the file compile, restates and proves the separation theorem
(and proves the refutation of the old statement), and pins the
toolchain. Verification is now reproducible in one command; see
Section~\ref{sec:status}.
\textbf{The mathematics of this paper is unchanged.} What changed is
what may be said about the machine verification of it.
\end{minipage}}
\end{center}
\medskip
...
% V7: the figure previously included here,
% fig_1A1_perelman_correspondence, does not exist --- not in this
% deposit, not in figures.py, not anywhere in AXLE. The source referenced
% it and therefore did not compile. Rather than substitute a different
% figure under its caption, it is withdrawn; Table~\ref{tab:perelman}
% below carries the correspondence in full.
\begin{quote}\small
\emph{Note (V7).} Earlier sources for this paper included a figure at
this point mapping the five-operator chain onto Ricci flow with
surgery. No such figure exists in the deposit or in
\texttt{figures.py}, and the \LaTeX{} source consequently did not
compile. It is withdrawn rather than replaced.
Table~\ref{tab:perelman} states the correspondence term by term.
Lean~4 status: operators $C,K,F,U$ are verified (Theorems~A--D);
operator~$E$ carries an open obligation (AXLE Issue~\#15).
The correspondence is structural, not causal.
\end{quote}
...
The verification artifact is \texttt{PrincipiaVol1.lean}, deposited
here and mirrored in AXLE at
\texttt{PrincipiaOrthogona1/PrincipiaVol1.lean}.
As of V7 it compiles and every theorem in it is kernel-checked.
...
\noindent
Three stages: \texttt{lake build}; then \texttt{\#print axioms} over
every named theorem; then a gate that refuses on \texttt{sorryAx} or on
any axiom outside the list above. The pin is part of the claim ---
``compiles under current Mathlib'' is not a checkable statement, and a
floating dependency is how the V3--V6 error survived four versions.

\medskip
\noindent
The following are among the 49:
...
\begin{enumerate}[label=\textbf{P\arabic*.},leftmargin=3em]
  ...
  \item Theorems A--D (existence, closure, normal form, stability),
    with concrete witnesses over $\mathbb{Z}$ for the
    non-commutativity of the chain (Theorem~5.3) --- nine theorems
    that earlier versions marked \emph{not machine-checked}.
  \item Separation theorem, V7 form: for one coherent direction of
    weight~1, transverse directions contracted below $e^{-2}$, and
    fewer than 33 directions in all,
    $\operatorname{Tr}(M^6) \neq 33$. The margin is wide --- the
    sixth-power trace lies within $31/4096$ of~1 --- and $n < 33$ is
    load-bearing: at $n = 33$ the trace equals~33 exactly. Shipped
    with a non-vacuity witness and with the proved refutation of the
    V3--V6 statement.
\end{enumerate}

\noindent
\textbf{What this does not cover.} The 49 theorems are arithmetic and
structural. They do not formalise the analytic content of this volume
--- the ODE integration behind Theorem~T1, the free-discontinuity
variational principle, or the Perelman correspondence. Those are
argued here and tracked below as open obligations.
Provenance claims of the form \emph{``from \texttt{X.lean} --- 0
sorry''} that appeared in earlier versions have been withdrawn: those
files have not been built either, and each claim returns only when its
file goes green.
...
\begin{enumerate}[label=\textbf{O\arabic*.},leftmargin=3em]
  \item \textbf{AXLE Issue \#12}, \emph{re-diagnosed in V7}.
    ...
    configuration manifolds --- remains open. The second was recorded
    as an ``eigenvalue API gap'' in the separation theorem; it was
    neither an API gap nor an unfinished proof, but a false statement
    and a lost exponent, and it is closed in V7.
    What is genuinely open in its place is the \emph{spectral
    reduction}: for an arbitrary real $M$,
    $\operatorname{Tr}(M^6) = \sum_i \lambda_i^6$ requires
    diagonalisability over $\mathbb{R}$, or a Jordan argument over
    $\mathbb{C}$. V7 states the theorem where that reduction has
    already been performed --- on the eigenvalue list, and on a
    diagonal matrix. That is a boundary of the statement, not a hole
    inside a proof, and no \texttt{sorry} is taken on its account.
...
\url{https://github.com/TOTOGT/vol1-proofs} --- run \texttt{bash tools/run.sh}.
Pinned to Lean \texttt{v4.14.0} and Mathlib \texttt{v4.14.0}
(rev \texttt{4bbdccd9c5f8}). Expected result: 49 theorems, 0
\texttt{sorry}, no axiom beyond \texttt{propext},
\texttt{Classical.choice}, \texttt{Quot.sound}.
The repository also carries the V6 file unchanged and the verbatim
81-error log of its first build, so the two can be diffed.
